...
- Confirm `CelebrityMatch.tsx` (/age-calculator) data source post-refactor: Supabase or still static `birthdays/*.ts` month files? If static, people shown can diverge from /results — the last consistency gap. Also decide the fate of `birthdayData.ts` / month files (TodaysBirthdays still uses them as a <20-results fallback).
- Confirm the country-gate mechanism in TodaysBirthdays (geo? locale?) — locale-based gating misfires for NRIs.

**Print/report**
- Check the 🇮🇳 flag emoji in the print Indian tag renders correctly in the Chromium print path (flag emoji are the least portable class; previous safe rendering was plain gold "Indian" text).
