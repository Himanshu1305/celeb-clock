...
## PART A of 2 — Phases 1–3 (Read + Astrological Data + Data Tests)

---

You have full permission to make all changes. Never ask for approval. Fix all TypeScript errors automatically. Work on develop branch only. Execute ALL phases from Part A then Part B sequentially. Each phase commits separately after tests pass.

**Goal:** Transform 598 thin celebrity pages into genuinely valuable content pages.

**CRITICAL RULES:**
1. Every astrological data value in Phase 2 is provided verbatim — do NOT invent or paraphrase any value
2. All tabs use conditional rendering `{activeTab === 'x' && <div>}` NOT CSS display:none
3. Bios stored in `src/data/celebrity-bios.json` — static JSON, not Supabase
4. `resolveJsonModule: true` must be in tsconfig BEFORE any JSON import
...
# 1. Check resolveJsonModule — MUST be present for JSON import
cat tsconfig.json | grep -A 40 '"compilerOptions"'

# 2. Read full CelebrityPage
cat src/pages/CelebrityPage.tsx

# 3. Read existing calculations
cat src/utils/celebrityCalculations.ts

# 4. Read utils
cat src/utils/celebrityUtils.ts | head -80

# 5. Check env
grep VITE_GEMINI .env.local 2>/dev/null && echo "KEY FOUND" || echo "KEY MISSING"

# 6. Check packages
cat package.json | grep '"p-limit"\|"tsx"\|"dotenv"\|"react"'

# 7. Print 3 real celebrity entries
python3 -c "
import re
with open('src/data/indianCelebrities.ts') as f:
    content = f.read()
objects = re.findall(r'\{[^{}]+\}', content)[:3]
for o in objects: print(o); print('---')
"
...
Print mapping:
```
resolveJsonModule in tsconfig: [true / MISSING]
CelebrityPage sections:        [list in order]
Celebrity data fields:         [exact field names]
GEMINI key:                    [FOUND/MISSING]
p-limit installed:             [YES/NO]
tsx installed:                 [YES/NO]
dotenv installed:              [YES/NO]
Baseline tests:                [X passing]
```

**Fix resolveJsonModule if missing:**
```bash
node -e "
const fs = require('fs');
const t = JSON.parse(fs.readFileSync('tsconfig.json','utf8'));
t.compilerOptions.resolveJsonModule = true;
fs.writeFileSync('tsconfig.json', JSON.stringify(t, null, 2));
console.log('resolveJsonModule added');
"
grep resolveJsonModule tsconfig.json
```

**STOP. No code until mapping is complete and resolveJsonModule is confirmed.**

---

## PHASE 2 — CREATE ASTROLOGICAL DATA FILE

**ALL data below is provided verbatim. Copy exactly. Do NOT invent any value.**

Create `src/data/astrologicalData.ts`:
...
npx tsx -e "import('./src/data/astrologicalData.ts').then(() => console.log('OK')).catch(e => console.error(e))" 2>&1

npx tsc --noEmit 2>&1 | head -10

git add src/data/astrologicalData.ts
git commit -m "feat(day8b/phase2): complete astrological data — 12 Western, 12 Vedic, 12 Chinese, 27 Nakshatra, 13 Life Path, runtime validation"
```

---

## PHASE 3 — WRITE DATA VALIDATION TESTS (38 tests)

Create `src/data/__tests__/astrologicalData.test.ts`:
...
```bash
npx vitest run src/data/__tests__/astrologicalData.test.ts --reporter=verbose 2>&1
```

All 38 TC-AD tests must pass. Fix any data errors in astrologicalData.ts before proceeding.

```bash
git add src/data/__tests__/astrologicalData.test.ts
git commit -m "test(day8b/phase3): 38 data validation tests — tarot cards, Vedic stones, mantras, Devanagari, Nakshatra numbers"
```
...
**Continue immediately with Part B (Phases 4-9).**
**Paste Part B contents now without stopping.**
