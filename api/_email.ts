// Shared email module — imported directly by server-side API routes.
// Exposes sendEmailDirect() so callers avoid HTTP self-fetch to /api/send-email.
//
...
function welcomeEmail(name: string, trialDays: number = 7): { subject: string; html: string } {
  const subjects = [
...
function trialExpiryEmail(name: string, hoursLeft: number = 24): { subject: string; html: string } {
  const subjects = [
...
function freeUserNudgeEmail(name: string, week: number, zodiacSign?: string, lifePathNumber?: number): { subject: string; html: string } {
  const variants = [
    {
...
function premiumUserNudgeEmail(name: string, week: number): { subject: string; html: string } {
  const variants = [
    {
...
function premiumActivatedEmail(name: string): { subject: string; html: string } {
  return {
...
export async function sendEmailDirect(payload: Record<string, unknown>): Promise<boolean> {
  ...
  if (!apiKey) {
    console.error('[sendEmailDirect] RESEND_API_KEY not configured');
    return false;
  }
  ...
  switch (type) {
    ...
    case 'data_deletion_request':
      ...
      break;
    default:
      console.error('[sendEmailDirect] unknown email type:', type);
      return false;
  }
  ...
  try {
    ...
    console.error('[sendEmailDirect] send error:', e);
    return false;
  }
}
