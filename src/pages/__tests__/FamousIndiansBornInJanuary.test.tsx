...
import { FamousIndiansBornInJanuary } from '../articles/FamousIndiansBornInJanuary';
afterEach(cleanup);
const renderArticle = () => render(
  <HelmetProvider><MemoryRouter><FamousIndiansBornInJanuary /></MemoryRouter></HelmetProvider>
);

describe('FamousIndiansBornInJanuary', () => {
  it('renders without crashing', () => {
    expect(() => renderArticle()).not.toThrow();
  });
  it('H1 contains the month name', () => {
    renderArticle();
    expect(document.querySelector('h1')?.textContent).toContain('January');
  });
  it('body contains a real celebrity born in January', () => {
    renderArticle();
    expect(document.body.textContent || '').toContain('Deepika Padukone');
  });
