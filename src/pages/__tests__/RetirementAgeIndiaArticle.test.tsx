...
import { RetirementAgeIndiaArticle } from '../articles/RetirementAgeIndiaArticle';
afterEach(cleanup);
const renderArticle = () => render(
  <HelmetProvider><MemoryRouter><RetirementAgeIndiaArticle /></MemoryRouter></HelmetProvider>
);

describe('RetirementAgeIndiaArticle', () => {
  it('renders without crashing', () => {
    expect(() => renderArticle()).not.toThrow();
  });
  it('H1 contains "retirement age"', () => {
    renderArticle();
    expect(document.querySelector('h1')?.textContent?.toLowerCase()).toContain('retirement age');
  });
  it('H1 contains "India"', () => {
    renderArticle();
    expect(document.querySelector('h1')?.textContent).toContain('India');
  });
  it('body mentions "EPFO"', () => {
    renderArticle();
    expect(document.body.textContent || '').toContain('EPFO');
  });
