...
function geneticCalc(p1: Pillar1Data): {
  ...
  if (pat.length === 0 && mat.length === 0) {
    return { adjustment: 0, weightedFamilyAge: 78, score: 'Average' };
  }
  ...
  return { adjustment: adj, weightedFamilyAge: wfa, score };
}
...
export function calculateLongevity(
  ...
  // ── Pillar 1: Genetics ──
  const { adjustment: genetic, weightedFamilyAge, score } = geneticCalc(pillar1);
  ...
  // Maximum potential: no smoking/drinking, excellent diet, heavy exercise, low stress,
  // optimal BP, 7-9 sleep, strong social, full genetic cap, all habits, community bonus
  const maxHealth = 0 + 0 + 4 + 5 + 1.5 + 1.5 + 0 + 2; // diet+ex+stress+bp+sleep+social
  const maximumPotential = Math.round((base + maxHealth + 8 + 6 + 0.5) * 10) / 10;

  // ── Factor breakdown ──
  const factors: FactorImpact[] = [];

  const add = (factor: string, emoji: string, currentValue: string, optimalValue: string, currentImpact: number, potentialGain: number, source: string) =>
    factors.push({ factor, emoji, currentValue, optimalValue, currentImpact, potentialGain, source });

  add('Tobacco Smoking', '🚬',
    quiz.smoking || 'Not answered', 'Non-smoker',
    smokeVal, smokeVal < 0 ? Math.abs(smokeVal) : 0,
    'WHO, 2023: Smoking is the leading preventable cause of death worldwide');

  add('Alcohol Consumption', '🍷',
    quiz.drinking || 'Not answered', 'None',
    drinkVal, drinkVal < 0 ? Math.abs(drinkVal) : 0,
    'CDC, 2023: No level of alcohol is safe for cancer risk');

  add('Physical Exercise', '🏋️',
    quiz.exercise || 'Not answered', 'Heavy (5+ sessions/week)',
    exVal, Math.max(0, 5 - exVal),
    'WHO, 2022: Moderate exercise reduces all-cause mortality by 31%');

  add('Diet Quality', '🥗',
    quiz.diet || 'Not answered', 'Excellent (whole foods)',
    dietVal, Math.max(0, 4 - dietVal),
    'Harvard School of Public Health: nutrient-dense diet adds 4+ years of healthy life');

  add('Stress Level', '🧠',
    `${quiz.stress}/10`, 'Low (1-2/10)',
    quiz.stress >= 8 ? -2.5 : quiz.stress >= 5 ? -0.5 : quiz.stress >= 3 ? 1 : 1.5,
    Math.max(0, 1.5 - (quiz.stress >= 8 ? -2.5 : quiz.stress >= 5 ? -0.5 : quiz.stress >= 3 ? 1 : 1.5)),
    'AHA, 2021: High daily stress increases cardiovascular disease risk by 40%');

  add('BMI / Body Weight', '⚖️',
    `${quiz.bmi}`, '18.5–24.9 (Normal)',
    quiz.bmi < 18.5 ? -2 : quiz.bmi > 30 ? -3 : quiz.bmi > 25 ? -1 : 0,
    quiz.bmi < 18.5 ? 2 : quiz.bmi > 30 ? 3 : quiz.bmi > 25 ? 1 : 0,
    'WHO, 2023: BMI 18.5–24.9 associated with lowest all-cause mortality');

  add('Blood Pressure', '❤️',
    quiz.bloodPressure || 'Not answered', 'Optimal (<120/80)',
    bpVal, Math.max(0, 1.5 - bpVal),
    'JNC8/AHA Guidelines, 2023: Optimal BP significantly reduces stroke and heart attack risk');

  add('Sleep Duration', '😴',
    quiz.sleepDuration || 'Not answered', '7–9 hours (optimal)',
    sleepVal, Math.max(0, -sleepVal),
    'NHS/National Sleep Foundation, 2023: <6 hrs/night has 12% higher all-cause mortality risk');

  add('Social Connections', '👥',
    quiz.socialConnections || 'Not answered', 'Strong',
    socVal, Math.max(0, 2 - socVal),
    'Harvard Study of Adult Development: strong relationships comparable to quitting smoking');

  add('Family Genetics', '🧬',
    `Family avg: ~${Math.round(weightedFamilyAge)} yrs`, 'Family avg >85 yrs',
    Math.round(genetic * 10) / 10, Math.max(0, Math.round((8 - genetic) * 10) / 10),
    'Karolinska Institute, 2017: Genetics account for 25-30% of longevity outcomes');
  ...
  add('Epigenetic Habits', '🌱',
    ...
    'WHO, 2023: Lifestyle factors account for up to 70% of longevity outcomes');
  ...
  if (quiz.heartDisease) {
    ...
    add('Heart Disease', '🫀',
      ...
      'ESC Guidelines 2023: Regular cardiac monitoring reduces adverse events by 20-30%');
  }
  if (quiz.diabetes) {
    ...
    add('Diabetes', '💉',
      ...
      'ADA 2023: Well-controlled blood sugar reduces CV complications by 50%');
  }
  if (quiz.hypertension) {
    ...
    add('Hypertension', '🩺',
      ...
      'AHA/ACC 2023: Treatment reduces stroke risk by 35-40%');
  }

  factors.sort((a, b) => b.potentialGain - a.potentialGain);

  return {
    baseLifeExpectancy: base,
    ...
    weightedFamilyAge:  Math.round(weightedFamilyAge * 10) / 10,
    selectedHabits:     pillar2.selectedHabits,
    quizSnapshot:       quiz,
...
// Recalculate from the stored snapshots with overridden slider/habit values
// Used by WhatIfSimulator to compute deltas in real time
